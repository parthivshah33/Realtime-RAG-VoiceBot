...
# script_dir = os.path.dirname(os.path.abspath(__file__)) # accessing current dir
...
#         return llm
#     except:
#         print("_Something went wrong while loading LLM_!")
...
#     except:
#         print("_Error loading embedding Client_!")
...
#         return index
#     except:
#         print("_Error preparing vectorDB_!")
...
#         try:
#             retrieved_chunks = vectorDB_client.query(vector=embeded_query,top_k=10,include_metadata=True)
...
# def run_rag_chain(retrieved_chunks,llm,user_query,memory,prompt=RAG_prompt):# -> Any | None:
#     try:
...
#         print(f"Memory variables : {memory.load_memory_variables({})}")
#         return rag_result

#     except:
#         print("_Error during running of Chain_!")


# def rag_pipeline(user_query,rag_prompt=RAG_prompt):
#     try :
...
#         pinecone_index = init_vectorDB()

#         retrievedChunks = get_topK_embeddings(user_query=user_query,embedding_client=embedding_client,vectorDB_client=pinecone_index)
#         # print("chunk loaded for rage")

#         memory = ConversationBufferMemory(memory_key="chat_history",return_messages=True)

#         rag_result = run_rag_chain(retrieved_chunks=retrievedChunks,llm=llm,user_query=user_query,prompt=rag_prompt,memory=memory)
#         # print("rag chain executed")
...
#     rag_result = rag_pipeline("What is deep learning?")
#     print(rag_result)
...
# RAG_prompt = PromptTemplate(
#     input_variables=['user_query', 'retrieved_chunks', 'chat_history'],
#     template='''...'''  # Your existing prompt content
# )
...
def run_rag_chain(retrieved_chunks, llm, user_query, memory, prompt=RAG_prompt):
    try:
        ...
        try:
            print(
                f"Memory variables: {memory.load_memory_variables({})['chat_history'][-3:]}")
        except:
            pass
        return rag_result
    except Exception as e:
